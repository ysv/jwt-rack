    ...
    describe 'iat' do
      describe 'will succeed' do
        describe 'with a valid iat and verify_iat unset' do
          ...
          let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify) }
        ...
        describe 'with a valid iat and verify_iat: false' do
          let(:payload) { { data: 'foo', iat: Time.now.to_i } }
          let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify, options: { verify_iat: false }) }
        ...
        describe 'with a valid iat and verify_iat: true' do
          let(:payload) { { data: 'foo', iat: Time.now.to_i } }
          let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify, options: { verify_iat: true }) }
