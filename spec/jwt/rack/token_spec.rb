...
describe JWT::Rack::Auth do
  ...
  describe 'receiving valid Authorization headers' do
    describe 'with unsigned (none) valid header and token' do
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: nil, verify: false, options: { algorithm: 'none' }) }
    ...
    describe 'with valid HMAC HS256 token (explicit)' do
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify, options: { algorithm: 'HS256' }) }
    ...
    describe 'with valid HMAC HS384 token' do
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify, options: { algorithm: 'HS384' }) }
    ...
    describe 'with valid HMAC HS512 token' do
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: verify, options: { algorithm: 'HS512' }) }
    ...
    describe 'with valid RSA RS256 key token' do
      ...
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: rsa_public, verify: verify, options: { algorithm: 'RS256' }) }
    ...
    describe 'with valid RSA RS384 key token' do
      let(:rsa_private) { OpenSSL::PKey::RSA.generate(2048) }
      let(:rsa_public)  { rsa_private.public_key }
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: rsa_public, verify: verify, options: { algorithm: 'RS384' }) }
    ...
    describe 'with valid RSA RS512 key token' do
      let(:rsa_private) { OpenSSL::PKey::RSA.generate(2048) }
      let(:rsa_public)  { rsa_private.public_key }
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: rsa_public, verify: verify, options: { algorithm: 'RS512' }) }
    ...
    describe 'with valid EC ES256 key token' do
      ...
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: ecdsa_pub, verify: verify, options: { algorithm: 'ES256' }) }
    ...
    describe 'with valid EC ES384 key token' do
      ...
      ecdsa_pub.private_key = nil
      let(:ecdsa_pub) { ecdsa_pub }
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: ecdsa_pub, verify: verify, options: { algorithm: 'ES384' }) }
    ...
    describe 'with valid EC ES512 key token' do
      ...
      ecdsa_pub.private_key = nil
      let(:ecdsa_pub) { ecdsa_pub }
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: ecdsa_pub, verify: verify, options: { algorithm: 'ES512' }) }
    ...
    describe 'with valid ED25519 key token' do
      ...
      let(:app) { JWT::Rack::Auth.new(inner_app, secret: public_key, verify: verify, options: { algorithm: 'ED25519' }) }
