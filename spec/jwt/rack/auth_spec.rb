...
describe JWT::Rack::Auth do
  ...
  let(:app) do
    JWT::Rack::Auth.new(inner_app, secret: secret)
  end
  ...
  describe 'initialization of' do
    describe 'secret' do
      ...
      describe 'with no secret: arg provided' do
        it 'raises ArgumentError' do
          expect { JWT::Rack::Auth.new(inner_app, {}) }.to raise_error(ArgumentError)
        end
      end

      describe 'with secret: arg of invalid type' do
        it 'raises ArgumentError' do
          expect { JWT::Rack::Auth.new(inner_app, secret: []) }.to raise_error(ArgumentError)
        end
      end

      describe 'with nil secret: arg provided' do
        it 'raises ArgumentError' do
          expect { JWT::Rack::Auth.new(inner_app, secret: nil) }.to raise_error(ArgumentError)
        end
      end
    ...
    describe 'verify' do
      describe 'with true arg' do
        let(:app) { JWT::Rack::Auth.new(inner_app, secret: secret, verify: true) }
    ...
    describe 'options' do
      ...
      describe 'when algorithm "none" and secret not nil but verify is false' do
        it 'raises an exception' do
          ...
          expect { JWT::Rack::Auth.new(inner_app, args) }.to raise_error(ArgumentError)
        end
      end
